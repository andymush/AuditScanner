    ...
    public function stream(string $id): StreamedResponse
    {
        ...
        return response()->stream(function () use ($report): void {
            ...
            while (true) {
                ...
                // Emit stage progress whenever meta.current_stage advances
                $currentStage = $report->meta['current_stage'] ?? 0;
                if ($currentStage !== $lastStage) {
                    echo 'data: '.json_encode([
                        'type' => 'stage',
                        'stage' => $currentStage,
                        'message' => $report->meta['stage_message'] ?? '',
                    ])."\n\n";
                    ob_flush();
                    flush();
                ...
                // Emit any findings inserted since the last poll
                $findings = $report->findings()->orderBy('created_at')->skip($emittedCount)->get();
                foreach ($findings as $finding) {
                    echo 'data: '.json_encode([
                        'type' => 'finding',
                        'engine' => $finding->confirmed_by[0] ?? 'gemini',
